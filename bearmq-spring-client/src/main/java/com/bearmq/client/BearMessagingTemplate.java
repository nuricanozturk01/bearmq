...
public class BearMessagingTemplate implements BearTemplate {
  ...
  private Optional<byte[]> readChunkedResponse(final DataInputStream dis) throws IOException {
    ...
    int off = 0;
    while (off < totalLen) {
      int chunkIdx = dis.readInt();
      int chunkLen = Math.min(DEFAULT_CHUNK_SIZE, totalLen - off);
  ...
  public Optional<byte[]> receive(String queue) throws BearMQException {
    final Map<String, Object> frame = Map.of(
            "operation", "dequeue",
            "queue", queue,
            "auth", getAuth()
  ...
  private Optional<byte[]> doReceive(Map<String, Object> frame) throws BearMQException {
  ...
  public void doSend(final Map<String, Object> frame) throws BearMQException {
    try (final Socket socket = new Socket(bearConfig.getHost(), bearConfig.getPort());
      ...
      throw new BearMQException("Could not send BearMQ message to " + bearConfig.getHost() + ":" + bearConfig.getPort(), e);
    }
  }
