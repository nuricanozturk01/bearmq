...
public class ThreadMetrics {
  ...
  public static void printUsage(final Thread thread) {
    final long id = thread.threadId();
    final long cpuTimeNs = THREAD_MX_BEAN.getThreadCpuTime(id);
    final long userTimeNs = THREAD_MX_BEAN.getThreadUserTime(id);
    ...
    /*
    System.out.printf(
        "Thread %s CPU=%.2f ms, User=%.2f ms | Heap=%.2f MB, NonHeap=%.2f MB OS:%s%n",
        thread.getName(),
